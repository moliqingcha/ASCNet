...
class DC(object):
    ...
    """
    def _coordinate_map(self, name):
        with tf.variable_scope(name+"/_coordinate_map"):
            
            # 离散化--生成 dilated rate_field ，维数转换为为 N×H×W×9
            rate = tf.cast(self.dilated_rate, "float32")
            rate_field = tf.tile(rate, [self.num_batch*self.height*self.width*self.num_points])
            new_rate_field = tf.reshape(rate_field, [self.num_batch, self.height, self.width, self.num_points])
    ...
    """
    函数dila_conv(self, inputs)用于进行dilated_convolution操作
    输入：input feature map
    输出：output feature map
    """
    def dila_conv(self, inputs, name, **kwargs):
        with tf.variable_scope(name+"/DeformedFeature"):
            x, y = self._coordinate_map( name)
