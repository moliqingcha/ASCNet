...
-------------------------------- ASPP --------------------------------------
函数pyramid_pooling()：构建pyramid_pooling模块；
-------------------------------- others --------------------------------------
...
#———————————————————————————— 请在这里设置参数 @_@ —————————————————————————#
    def __init__(self, sess, conf, is_train):
        ...
        # 设置下采样程度：选项为 8,16（注意：原文中的OS是8，所以如果不是计算资源实在短缺，请使用8）
        self.output_stride = 8
        
        # 设置pooling size
        self.pool_size = [1, 2, 3, 6]
        #—————————————————————————————————————————#
        # 设置block深度，只可以设成4
        self.block_depth = 4
...
#———————————————————————————— 整体的网络架构 —————————————————————————#  
    ...
    def inference(self, inputs):
        ...
        outputs = inputs
        name = 'resnet'
        outputs, pred2 = self.resnet(outputs, rate_field, name) 
                
        #——————————————  step：3  ——————————————#—— pyramid_pooling ———#
        name = 'pyramid_pooling'
        outputs = self.pyramid_pooling(outputs, rate_field, name)
        
        #——————————————  step：4  ——————————————#—— final ———#
        name = 'final'  
        outputs = self.down_conv_func()(outputs, rate_field, 512, (3, 3), name+'/conv1', is_train=self.is_train, bias=False)
        ...
        #——————————————  step：5  ——————————————#—— 辅助loss ———#
        name = 'additional'
        pred2 = self.down_conv_func()(pred2, rate_field, 512, (3, 3), name+'/conv1', is_train=self.is_train, bias=False)
        pred2 = ops.conv2d(pred2, rate_field, self.conf.class_num, (1, 1), scope=name+'/conv_final', is_train=self.is_train, bias=False)
        pred2 = tf.image.resize_bilinear(pred2, size=(self.output_shape[1],self.output_shape[2]), align_corners=True, name=name+'/bilinear')
           
        return outputs,rate_field,pred2
#———————————————————————————— resnet-50--101--152 —————————————————————————#     
    """
    函数功能：把resnet打包成一个组件
    ...
    def resnet(self, inputs, rate_field, name):
        ...
        #——————————————  step：2  ——————————————#——7个block部分———#
        # 需要满足的条件：1）block内最后一个bottle输入需要的stride；
        for block_index in range(self.block_depth):
            for bottle_index in range(self.bottleneck_num[block_index]):
                ...
                # 输出用于辅助loss的特征图
                if block_index==2 and bottle_index==(self.bottleneck_num[block_index]-2):
                    pred2 = outputs
            print("———————————————(￣︶￣)——————————————————")
        
        return outputs, pred2
#———————————————————————————— start_block —————————————————————————#     
    """
    函数功能：网络最开始的卷积和池化
    输入：input feature map
...
#—————————————————————————————— pyramid pooling ———————————————————————————# 
    """
    函数功能：pyramid pooling
    输入：input feature map
    输出：output feature map
    注意：
    1）使用的padding方式是VALID，防止same带来的不对准问题；
    """
    def pyramid_pooling(self, inputs, rate_field, name):
        
        #——————————————  step：1  ——————————————#——pyramid-pooling部分———#
        height = inputs.shape[1].value
        width = inputs.shape[2].value
        num_outputs = inputs.shape[3].value//4
        
        # 4个池化
        features = []
        features.append(inputs)
        for i in range(4):
            pool = ops._avg_pool2d(inputs, height//self.pool_size[i], name+'pool'+str(i+1), stride=height//self.pool_size[i], padding='VALID')
            pool = ops.conv2d(pool, rate_field, num_outputs, (1, 1), scope=name+'/conv'+str(i+1), is_train=self.is_train, bias=False)
            pool = tf.image.resize_bilinear(pool, size=(height,width), align_corners=True, name=name+'/bilinear'+str(i+1))
            features.append(pool)
            
        #——————————————  step：2  ——————————————#——concat整合部分———#
        outputs = tf.concat(features, axis=3, name=name+'/concat')
